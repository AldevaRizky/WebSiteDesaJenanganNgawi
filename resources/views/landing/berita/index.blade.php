...
@section('content')
    ...
    <div class="container mx-auto">
        ...
        <div class="flex flex-wrap justify-center gap-3 px-6 mb-8">
            <a href="{{ route('landing.berita') }}" class="px-6 py-2 rounded-full font-semibold transition-all duration-300 {{ !request('kategori') ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-700 border-2 border-gray-300 hover:border-blue-500 hover:text-blue-500' }}">
                Semua
            </a>
            @foreach($kategoris as $kategori)
            <a href="{{ route('landing.berita', ['kategori' => $kategori->slug]) }}" class="px-6 py-2 rounded-full font-semibold transition-all duration-300 {{ request('kategori') == $kategori->slug ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-700 border-2 border-gray-300 hover:border-blue-500 hover:text-blue-500' }}">
                {{ $kategori->nama }}
            </a>
            @endforeach
        </div>
